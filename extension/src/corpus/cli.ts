...
async function main(): Promise<void> {
  ...
  if (command === "scan" && args.length === 4) {
  ...
  } else if (command === "provision" && (args.length === 5 || args.length === 6 || args.length === 7)) {
    const repositories = await readJson<RepositoryRegistration[]>(args[0]!);
    const registration = repositories.find(({ repositoryId }) => repositoryId === args[1]);
    if (registration === undefined) throw new Error(`Unknown repository registration: ${args[1]}`);
    ...
    const eligibleLimit = args[6] === undefined ? 10 : Number(args[6]);
    const evidence = await provisionRepository(resolve(args[3]!), registration, drafts, prior, eligibleLimit);
    ...
    process.stderr.write([
      ...
      "  npm run r7:corpus -- provision <repositories.json> <repository-id> <preflight.json> <git-repository> <new-evidence.json> [prior-evidence.json] [eligible-limit]",
