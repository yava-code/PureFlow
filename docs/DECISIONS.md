...
### Decision: Vibe Code Comprehension Gate
*   **Context:** The user requested a feature similar to Cursor's AI coding, but without losing the core value of maintaining developer engineering skills ("vibe coding").
*   **Decision:** Added a `"vibecode"` mode. The AI generates the implementation (`codeSnippet`) but pairs it with an `understandingQuestion`. The "Apply to Editor" button is disabled in `MentorDock.tsx` until the user provides a response to the question.
*   **Consequence:** Merges the velocity of AI code generation with a forced checkpoint that ensures developers read, parse, and reason about the code before it enters their repository.
